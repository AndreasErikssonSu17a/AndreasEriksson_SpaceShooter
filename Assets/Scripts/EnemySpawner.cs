...
public class EnemySpawner : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		if (cooldown <= 0)
		{
			...
			GameObject enemy = Instantiate(enemyPrefab, new Vector2(rbody2D.position.x, rbody2D.position.y + Random.Range(maxSpawnPoint, minSpawnPoint)), Quaternion.Euler(0,0,-90f));
			cooldown = 60f / spawnRate;		//60 / 60 = 1

            enemy.GetComponent<BasicEnemy>().speed *= enemySpeedMult;
		}
		else
		{
